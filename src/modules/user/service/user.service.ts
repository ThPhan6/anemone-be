...
export class UserService extends BaseService<User> {
   ...
   */
  async updateCmsUserById(userId: string, userData: UpdateUserDto): Promise<boolean> {
    try {
      // 1. Validate and get current user data
      const cognitoUser = await this.cognitoService.getCMSUserByUserId(userId);

      if (!cognitoUser) {
        throw new Error('User not found');
      }

      // 2. Update user in Cognito
      const updatedCognitoUser = await this._updateCognitoUser(cognitoUser, userData);

      // 3. Sync with database
      await this._syncUserWithDatabase(cognitoUser.id, updatedCognitoUser, userData);
  ...
  /**
   * Updates user in Cognito and returns updated user data
   * @throws Error if update fails
   */
  private async _updateCognitoUser(cognitoUser: any, userData: UpdateUserDto) {
    // Update in Cognito
    const updateResult = await this.cognitoService.updateUser({
      ...
      role: userData.role,
      enabled: cognitoUser.enabled === userData.enabled ? undefined : userData.enabled,
    });
    ...
    const updatedCognitoUser = await this.cognitoService.getCMSUserByUserId(cognitoUser.id);
   ...
   */
  private async _syncUserWithDatabase(
    userId: string,
    cognitoUser: any,
    userData: UpdateUserDto,
  ): Promise<void> {
    const existingUser = await this.repository.findOne({ where: { user_id: userId } });
    ...
    const userDataForDb = this._prepareUserDataForDb(cognitoUser, existingUser, userData);
    await this.repository.update({ user_id: userId }, userDataForDb);
  }

  /**
   * Prepares user data for database update
   * Combines Cognito data with existing user data and new updates
   */
  private _prepareUserDataForDb(
    cognitoUser: any,
    existingUser: User,
    userData: UpdateUserDto,
  ): Partial<User> {
   ...
   */
  private async _getUpdatedUser(user_id: string): Promise<User> {
  ...
  async createCmsUser(userData: CreateUserDto): Promise<boolean> {
    try {
